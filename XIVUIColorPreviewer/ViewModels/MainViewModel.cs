...
public partial class MainViewModel : ObservableObject
{
    ...
    public UIColorDataService ColorDataService { get; } = new();

    public List<string> ThemeNames => ColorDataService.ThemeNames;
    ...
    {
        IsLoading = true;
        await ColorDataService.LoadAsync();
        await _schemeService.LoadAsync();

        foreach (var scheme in _schemeService.Schemes) Schemes.Add(scheme);

        // Set defaults
        if (ColorDataService.RowNumbers.Count > 0)
        {
            ForegroundColorRow = ColorDataService.RowNumbers[0];
            StrokeColorRow     = ColorDataService.RowNumbers[0];
        }
    ...
    private async Task SaveSchemeAsync()
    {
        ...
        var scheme = new ColorScheme
        {
            ...
            TextBlocks = TextBlocks.Select
            (tb => new TextBlockConfigData
                {
        ...
        Schemes.Clear();
        foreach (var s in _schemeService.Schemes) Schemes.Add(s);
    ...
    private async Task DeleteSchemeAsync(ColorScheme? scheme)
    {
        ...
        if (SelectedScheme == scheme) SelectedScheme = null;
    }
